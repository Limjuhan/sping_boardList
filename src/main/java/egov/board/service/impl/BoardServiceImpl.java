...
public class BoardServiceImpl extends EgovAbstractServiceImpl implements BoardService{
	...
	public void saveBoard(HttpServletRequest request) throws Exception {
		...
		String convertuid = "";
		String originalEx = "";//사용자가 업로드 요청한 파일의 확장자
		String filePath = ""; //파일을 전송할 위치
		...
		if(request instanceof MultipartHttpServletRequest) {
			...
			for(MultipartFile file: files.values()) {
				if(!"".equals(file.getOriginalFilename())) {
					...
					originalEx = file.getOriginalFilename().
							substring(file.getOriginalFilename().lastIndexOf(".") + 1);
					convertuid = convertuid + "." + originalEx;//파일명
					//서버의 파일경로
					...
					paramMap2.put("in_fileurl", "http://localhost:8080/Egov_WEB/boardView/image.do?file=" + convertuid);
